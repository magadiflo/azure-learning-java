...
| #  | Ícono | Servicio              | Categoría                | ¿Para qué sirve?                                                      | 🧭 Razonamiento del orden                                                                                                         | 🖥️ Alternativa local para aprender                                                                                                                                                                                                                                                                                                                                                   |
|----|-------|-----------------------|--------------------------|-----------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 01 | 🚀    | **Azure App Service** | Cómputo / PaaS           | Desplegar y ejecutar aplicaciones web y APIs sin gestionar servidores | Va primero porque es donde *vive* tu app Spring Boot. Entender esto da contexto a todos los demás servicios que tu app consumirá. | ✅ **No necesitas Azure.** Ejecuta tu app Spring Boot directamente con `mvn spring-boot:run` o en un contenedor Docker local. El comportamiento es idéntico — App Service solo es el lugar donde vive en la nube.                                                                                                                                                                      |
| 02 | 🗃️   | **Azure SQL**         | Base de datos relacional | Base de datos SQL gestionada, compatible con SQL Server               | Tu app ya corre en App Service, ahora necesita persistir datos relacionales. Es lo más cercano a tu mundo actual.                 | ✅ **SQL Server local o Docker.** Azure SQL es SQL Server en la nube — el motor es el mismo. Usa **SQL Server Express** (gratuito) + **SSMS** que ya tienes instalado. O bien con Docker: `docker run -e "ACCEPT_EULA=Y" -e "SA_PASSWORD=MiPassword123!" -p 1433:1433 mcr.microsoft.com/mssql/server:2022-latest`                                                                      |
| 03 | 🌌    | **Cosmos DB**         | Base de datos NoSQL      | Base de datos distribuida y multi-modelo                              | Contraste natural con Azure SQL: relacional vs. NoSQL, estudiados uno seguido del otro.                                           | ✅ **Cosmos DB Emulator.** Microsoft ofrece un emulador oficial gratuito para Windows que replica el comportamiento completo de Cosmos DB. Descárgalo buscando **"Azure Cosmos DB Emulator download"**. También disponible como imagen Docker: `docker pull mcr.microsoft.com/cosmosdb/linux/azure-cosmos-emulator`                                                                    |
| 04 | 📦    | **Azure Storage**     | Almacenamiento           | Blobs, archivos, colas y tablas en la nube                            | Tu app ya tiene datos estructurados; ahora necesita guardar archivos, imágenes y documentos. Concepto transversal a casi todo.    | ✅ **Azurite + Azure Storage Explorer.** Azurite es el emulador oficial gratuito de Azure Storage. Instala con `npm install -g azurite` y luego `azurite`. Para ver los datos visualmente instala **Azure Storage Explorer** (gratuito). Simula Blob, Queue y Table Storage completos.                                                                                                 |
| 05 | ⚡     | **Azure Functions**   | Serverless / Cómputo     | Ejecutar código sin gestionar servidores                              | Complemento de App Service para lógica puntual y tareas sin estado. Tiene más sentido después de ver App Service.                 | ✅ **Azure Functions Core Tools.** Herramienta oficial gratuita para crear y ejecutar funciones completamente en local. Instala con `npm install -g azure-functions-core-tools@4`. Combínala con Azurite para los triggers de Blob y Queue.                                                                                                                                            |
| 06 | 📨    | **Service Bus**       | Mensajería               | Cola de mensajes empresarial (pub/sub)                                | Comunicación asíncrona entre microservicios. Muy relevante para arquitecturas reactivas con WebFlux.                              | ✅ **Service Bus Emulator (Docker).** Microsoft lanzó en 2024 un emulador oficial. Ejecútalo con Docker: `docker pull mcr.microsoft.com/azure-messaging/servicebus-emulator:latest`. Incluye soporte para colas y topics. Úsalo junto con **Service Bus Explorer** (GUI gratuita en GitHub) para ver mensajes visualmente.                                                             |
| 07 | 🌊    | **Event Hubs**        | Streaming de eventos     | Ingesta masiva de eventos en tiempo real                              | Complemento natural del Service Bus: mensajería puntual vs. streaming masivo, juntos tienen más sentido.                          | ✅ **Event Hubs Emulator (Docker).** También lanzado en 2024 con imagen oficial: `docker pull mcr.microsoft.com/azure-messaging/eventhubs-emulator:latest`. Alternativamente, dado que Event Hubs es compatible con el protocolo Kafka, puedes usar **Apache Kafka en Docker** (`docker pull apache/kafka`) como sustituto completo para aprender los conceptos.                       |
| 08 | 🚪    | **API Management**    | API Gateway              | Publicar, proteger y monitorear APIs                                  | Va último porque para apreciarlo necesitas entender todos los servicios que expone y gestiona.                                    | ⚠️ **Sin emulador local oficial.** APIM no tiene un equivalente ejecutable en local. Para aprender los conceptos puedes usar **Kong Gateway** (open source, `docker pull kong`) o **nginx** como API Gateway local. Para practicar las políticas XML de APIM específicamente, necesitarías el tier **Consumption** en Azure real (paga por llamada, prácticamente gratis en pruebas). |
